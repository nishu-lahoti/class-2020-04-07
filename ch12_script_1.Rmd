...
x %>%
  filter(tier == 2) %>%
  select(name, tier, earnings, sat, price) %>%
  arrange(desc(sat))
...
faculty <- x %>%
  select(name, tier, earnings, faculty) %>%
  summarize(corr_fac = cor(faculty, earnings))

sat <- x %>%
  select(name, tier, sat, earnings) %>%
  summarize(corr_sat = cor(sat, earnings))
...
- Treatment = average SAT score (or is the treatment a change in financial aid policy, with control being the current policy?) 

- Outcome = median earning of students 10 years after they enter college
